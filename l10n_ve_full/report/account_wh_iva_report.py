    ...
    def _get_report_values(self, docids, data=None):
            ...
            if wh_iva.state == 'done':
                if (wh_iva.type == 'in_invoice' or wh_iva.type == 'in_refund' or wh_iva.type =='in_debit'):
                    if wh_iva.wh_lines:
                        ...
                        base_exent = ' '
                    for line in wh_iva.wh_lines:
                        if line.type == 'in_invoice' or line.type == 'in_refund' or line.type == 'in_debit':
                        ...
                        if line.invoice_id.move_type == 'in_refund':
                            inv_refund = line.invoice_id.supplier_invoice_number
                            inv_nro_fact = line.invoice_id.invoice_reverse_purchase_id.supplier_invoice_number
                            inv_nro_ctrl = line.invoice_id.nro_ctrl
                        elif line.type == 'in_debit':
                            factura_origin = self.env['account.move'].search([('id','=', line.invoice_id.debit_origin_id.id)])
                            inv_debit = line.invoice_id.supplier_invoice_number
        ...
        if data['form'].state == 'done':
            if data['form'].wh_lines.invoice_id.currency_id.id == data['form'].company_id.currency_id.id :
                total_doc = sum(data['form'].wh_lines.invoice_id.mapped('amount_total'))
            elif data['form'].wh_lines[0].invoice_id.currency_id != data['form'].company_id.currency_id:
