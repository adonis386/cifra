...
class AccountMove(models.Model):
    ...
    comprobante_muni = fields.Char(string='NRO. COMPROBANTE', copy=False)

    tipo_contribuyente_mun = fields.Selection([
        ('D', 'Domiciliado'),
        ('T', 'Transeúnte')], string='Tipo de Contribuyente', related='partner_id.tipo_contribuyente_mun')
    codigo_asignado = fields.Char('Código Asignado', related='partner_id.codigo_asignado', readonly=True)
    ...
    sequence_nro_ctrl_id = fields.Many2one('ir.sequence', string='Secuencia Nro de Control', related='journal_id.sequence_nro_ctrl_id')

    sequence_nro_ctrl_next = fields.Integer(string='Próximo Nro de Control', related='journal_id.sequence_nro_ctrl_next')
    ...
    def _post(self, soft=True):
        ...
        for rec in self:
            if rec.sequence_nro_ctrl_id and not rec.nro_ctrl:
                rec.nro_ctrl = rec.sequence_nro_ctrl_id.next_by_id()
        return res
        ...
        for res in self:
            ...
            if not res.journal_id.eliminar_impuestos:
                ...
                if res.company_id.partner_id.islr_withholding_agent and res.partner_id.islr_withholding_agent:
                    if res.state == 'posted' and not res.islr_wh_doc_id and len(res.invoice_line_ids.filtered(lambda x: x.concept_id.withholdable)) > 0 and res.move_type in ['out_invoice', 'out_refund', 'in_invoice', 'in_refund']:
    ...
    def gerenrar_retencion_masiva(self):
        islr_wh_doc_id = self._create_islr_wh_doc()
        if islr_wh_doc_id:
            self.write({'islr_wh_doc_id': islr_wh_doc_id.id})

    def gerenrar_retencion_masiva_iva(self):
        iva_wh_doc_id = self.action_wh_iva_create()
    ...
    def ret_and_reconcile(self, pay_amount, pay_account_id,
        ...
        #get account pagable
        src_account_id = self.line_ids.filtered(lambda x: x.account_id.account_type in ('asset_receivable', 'liability_payable')).account_id.id
        ...
        l1 = {
            ...
            #'amount_residual': direction * pay_amount,
        ...
        deb = l1['debit']
        cred = l1['credit']
        l2[0][2].update({'credit': deb})
        l2[0][2].update({'debit': cred})
        #l2[0][2].update({'credit': deb})

        print("l1",l1)
        print("l2",l2)
        if module_dual_currency:
            l2[0][2]['amount_residual_usd'] = direction * -1 * (pay_amount / invoice.tax_today)
        lines += l2
    ...
    def _create_islr_wh_doc(self):
        ...
        if wh_ret_code:
            ...
            acc_part_id = rp_obj._find_accounting_partner(self.partner_id)
            if self[0].move_type in ('out_invoice', 'out_refund'):
                acc_id = acc_part_id.property_account_receivable_id.id
                wh_type = 'out_invoice'
            ...
            values = {
                ...
                'account_id': acc_id,
                'type': self[0].move_type,
                'journal_id': journal.id,
                'date_uid': self[0].date,
                'company_id': self[0].company_id.id,
                'date_ret': self[0].date
            }
            if self[0].company_id.propagate_invoice_date_to_income_withholding:
                values['date_uid'] = self[0].invoice_date
            ...
            if self[0].company_id.automatic_income_wh is True:
                wh_doc_obj.write(
    ...
    def _get_move_lines2(self,
        ...
        amount_islr_ret = to_wh.islr_wh_doc_id.amount_total_ret
        rec = to_wh.islr_wh_doc_id.journal_id.default_islr_account
        # concept_id.property_retencion_islr_receivable
        pay = to_wh.islr_wh_doc_id.journal_id.default_islr_account
        if inv_brw.move_type in ('out_invoice', 'out_refund'):
        ...
        if not acc:
            raise UserError(
                "Falta la cuenta en el impuesto! \nEl diario de [%s] tiene las cuentas faltantes. Por favor, rellene los campos que faltan para poder continuar" % (
                    to_wh.islr_wh_doc_id.journal_id.name))

        res.append((0, 0, {
            'debit': direction * amount_islr_ret < 0 and - direction *
                     amount_islr_ret,
            'credit': direction * amount_islr_ret > 0 and direction *
                      amount_islr_ret,
            'account_id': acc,
            'partner_id': acc_part_id.id,
            ...
            'name': name.strip() + ' - ISLR: ' + to_wh.islr_wh_doc_id.name.strip()
        }))
            ...
            if rec.municipal_rate_id:
                if rec.company_id.aplicar_sobre == 'amount_untaxed':
                    ...
                    if rec.currency_id != rec.company_id.currency_id:
                        rec.monto_alicuota = rec.monto_alicuota * rec.tax_today
    ...
    def generate_wh_muni(self):
        for rec in self:
            ...
            if rec.account_move_ret_muni_id and not rec.wh_muni_id:
                #gerenrar el docmento de retencion municipal account.wh.municipal.docs
                line_ids = [(0, 0, {
                    'invoice_id': rec.id,
                })]
                wh_muni_doc = self.env['account.wh.municipal.docs'].create({
                    'name': rec.comprobante_muni or '',
                    'partner_id': rec.partner_id.id,
                    'date': rec.date,
                    'company_id': rec.company_id.id,
                    'state': 'draft',
                ...
                if wh_muni_doc:
                    wh_muni_doc._compute_periodo_fiscal()
                    rec.wh_muni_id = wh_muni_doc.id
