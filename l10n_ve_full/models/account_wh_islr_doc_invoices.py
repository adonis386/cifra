...
class AccountWhIslrDocInvoices(models.Model):
    ...
    base_ret = fields.Float(string='Cantidad base', digits=(16, 2), help="Monto a partir del cual se calculará una retención", compute='_amount_all_base_ret')
    ...
    def _amount_all_base_ret(self):
        for rec in self:
            rec.base_ret = sum(rec.islr_xml_id.mapped('base') if rec.islr_xml_id else rec.iwdl_ids.mapped('base_amount'))
    ...
    def _get_wh(self, iwdl_id, concept_id):
        ...
        if iwdl_id.invoice_id.move_type in ('in_invoice', 'in_refund'):
            ...
            rate_tuple = self._get_rate(concept_id, residence, nature, base=base, inv_brw=iwdl_id.invoice_id)
            print('rate_tuple', rate_tuple)
            if rate_tuple[7]:
                apply_income = True
            else:
                apply_income = (apply_income and
