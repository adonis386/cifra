...
class AccountWhIva(models.Model):
    ...
    def get_reconciled_move(self):
        ...
        dominio = [('move_id', 'in', awil_brw.move_id.ids),
                   ('reconciled', '=', True)]
    ...
    def confirm_check(self):
        ...
        '''
        if self.type == 'out_invoice' and not self.number:
            raise UserError(_("Número de comprobante no establecido!"))
        if self.total_tax_ret == 0:
        ...
        if (not self.check_wh_lines() or
            ...
            self.wh_lines.invoice_id.write({'wh_iva': True,'iva_number_asignado':self.number})
        return True
