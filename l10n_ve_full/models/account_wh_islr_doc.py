...
class AccountWhIslrDoc(models.Model):
    ...
    def _get_amount_total(self):
        ...
        total = sum(self.invoice_ids.mapped('amount_islr_ret'))
        return total
    ...
    date_uid = fields.Date(
        'Fecha de retención',  help="Fecha del vale")
    ...
    send_reten = fields.Boolean(string='Enviado al proveedor', default=False, tracking=True)
    ...
    def compute_amount_wh(self, islr_wh_doc_id):
        ...
        for iwdi_brw in iwd_brw.invoice_ids:
            ...
            iwdi_brw.amount_islr_ret = sum(iwdi_brw.islr_xml_id.mapped('wh')) if iwdi_brw.islr_xml_id else pruee.get('amount_islr_ret', 0.0)
            iwdi_brw.base_ret = sum(iwdi_brw.islr_xml_id.mapped('base')) if iwdi_brw.islr_xml_id else pruee.get('base_ret', 0.0)
        ...
        iwd_brw.amount_total_ret = sum(iwd_brw.invoice_ids.mapped('amount_islr_ret'))
    ...
    def action_confirm(self):
        ...
        """
        if self.amount_total_ret == 0:
            raise UserError(
                "Error de Validación \n¡No se puede confirmar una retención con monto 0!")
        context = self._context or {}
        ids = isinstance(self.ids, (int)) and [self.ids] or self.ids
    ...
    def action_cancel(self):
        ...
        # return True
        for rec in self:
            rec.get_reconciled_move()
            rec.cancel_move()
            rec.action_cancel_process()

            rec.env['account.wh.islr.doc'].write(
    ...
    def get_reconciled_move(self):
        ...
        dominio = [('move_id', 'in', iwdi_brw.move_id.ids),
                   ('reconciled', '=', True)]
