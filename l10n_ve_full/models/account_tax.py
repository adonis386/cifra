...
class AccountTax(models.Model):
    ...
    def eliminar_productos(self):
        #fintar productos con precio 0 en invoice_line_ids
        existe = r.invoice_line_ids.filtered(lambda x: x.price_unit == 0)
        if existe:
            #pasar factra a borrador
            r.button_draft()
            existe.unlink()
            if r.dian_document_lines:
                r.dian_document_lines.unlink()
            r.button_validate()
