...
class TxtIva(models.Model):
    ...
    journal_id = fields.Many2one(
        ...
        help="Diario para el asiento contable")
    ...
    def check_txt_ids(self):  # , cr, uid, ids, context=None
        ...
        for awi in self:
            ...
            #verifica que no existan facturas en borrador
            fac = awi.txt_ids.filtered(lambda x: x.invoice_id.state == 'draft')
            if fac:
                raise UserError("Valores incorrectos! \nNo se puede generar un archivo TXT con facturas en borrador.")
            for line in awi.txt_ids:
                if line.amount_withheld == 0:
                    raise UserError("Valores incorrectos! \nEl monto retenido debe ser mayor que cero.")
        return True
    ...
    def action_generate_lines_txt(self):
        ...
        #verificar que vouchers no exista en ningun registro del modelo account.wh.iva.txt.line
        vouchers_in_lines = txt_iva_obj.search([('voucher_id', 'in', vouchers.ids)])
        #si existe eliminar de la lista vouchers
        if vouchers_in_lines:
            for voucher in vouchers_in_lines:
                vouchers = vouchers - voucher.voucher_id
        amount_amount = 0
        base_base = 0
    ...
    def generate_pay(self):
        ...
        #obtener cuenta de retencion
        account_id = self.txt_ids[0].voucher_id.journal_id.default_iva_account
        if not self.company_id.partner_seniat_id:
            raise UserError('La compañía no tiene configurado el socio seniat')

        if self.move_id or self.payment_id:
            raise UserError('Ya se generó el asiento contable y el pago')
        ...
        move = self.env['account.move'].create({
            ...
            'line_ids': [(0, 0, {
                ...
                'debit': self.amount_total_ret,
                'credit': 0.0,
            }), (0, 0, {
                'name': self.name,
                'account_id': self.company_id.partner_seniat_id.property_account_payable_id.id,
                'debit': 0.0,
                'credit': self.amount_total_ret,
                'partner_id': self.company_id.partner_seniat_id.id,
            })]
        })
        move.action_post()
        ...
        payment = self.env['account.payment'].create({
            ...
            'date': fields.Date.today(),
            'ref': self.name,
            'destination_account_id': self.company_id.partner_seniat_id.property_account_payable_id.id,
        })
